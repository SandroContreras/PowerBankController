...
class BatteryManager:
    ...
    def SetWindowSize(self):		## Lithium batteries don't charge linearly
        if self.battery_voltage < 3.7:
            ...
            self.windowSize = 20
        elif self.battery_voltage < 4.01:
            self.windowSize = 15
        elif self.battery_voltage < 4.05:
            self.windowSize = 8
        elif self.battery_voltage >= 4.05 and self.battery_voltage < 4.061:
            self.windowSize = 8
        elif self.battery_voltage >= 4.061:
            self.windowSize = 5
    ...
    def SOCtable(self, previous_battery_percent_str, battery_percent_str):
        VoltageRange = [2.8, 3.0, 3.1, 3.2, 3.3, 3.35, 3.4, 3.45, 3.50, 3.55, 3.6, 3.7, 3.75, 3.8, 3.9, 3.95, 4.0, 4.03, 4.05, 4.06, 4.07, 4.08]
        Percentage =   [  2,   5,   7,  10,  13,   15,  20,   25,   30,   35,  40,  50,   60,  65,  70,   75,  80,   85,   90,   95,   98,  100]
        print("SMA in SOC = " + str(self.SMA_battery_voltage))
        
        if (self.SMA_battery_voltage < 2.8):
            Index = 0
        elif (self.SMA_battery_voltage > 4.08) :
            Index = len(VoltageRange) - 1
        else:
            Index =  bisect.bisect_right(VoltageRange, self.SMA_battery_voltage) - 1
            
#         if (previous_battery_percent_str != "" and battery_percent_str != ""):
#             return self.sticky(previous_battery_percent_str, battery_percent_str)
#         else:
...
#     def sticky(self, previous_battery_percent_str, battery_percent_str):
#         flag = any(voltage == 4.06 for voltage in self.movingAvg)
#         
#         if (flag):
#             if (int(previous_battery_percent_str) >= 95 and Percentage[Index] < int(previous_battery_percent_str)):
#                 return previous_battery_percent_str
#             elif (int(previous_battery_percent_str) >= 95 and Percentage[Index] > int(previous_battery_percent_str)):
#                 return Percentage[Index]
#         else:
#             return Percentage[Index]
...
class OledUI():
    ...
    def DrawBatteryPercentage(self, BatteryMethods, battery_percent_str):
        ...
        if (BatteryMethods.SMA_battery_voltage < 3.2):	## Shift Percent Symbol inwards towards the Single Digit Value
            self.oled.text(self.percentSymbol, 65, 2)
            self.oled.text(battery_percent_str, 57, 2)
        elif (BatteryMethods.SMA_battery_voltage < 4.08):	## Display Battery Percentage for Double Digit Value
            self.oled.text(self.percentSymbol, 65, 2)
            self.oled.text(battery_percent_str, 50, 2)
        elif (BatteryMethods.SMA_battery_voltage < 4.09 or BatteryMethods.SMA_battery_voltage > 4.09):	## Triple digit value display
            self.oled.text(self.percentSymbol, 75, 2)
            self.oled.text(battery_percent_str, 50, 2)
    ...
    def DrawChargingSymbol(self, BatteryMethods):
        ...
        if (VoltageChangeRate > 0.0003 and self.previous_SMA_battery_voltage != 0):		## The Charging symbol will only appear if the Bank is Charging
            if (BatteryMethods.SMA_battery_voltage < 4.07):
                ...
                ## Create Charging Symbol
                self.oled.vline(93, 0, 16, 1)
                self.oled.hline(93, 6, 8, 1)	## Create horizontal line at halfway point at charging symbol
                self.oled.hline(86, 9, 8, 1)	## Create horizontal line at halfway point at charging symbol

                ## Inner Symbol Fillings
                self.oled.fill_rect(88, 6, 5, 4, 1)
                self.oled.fill_rect(93, 6, 4, 4, 1)
                self.oled.fill_rect(91, 2, 2, 4, 1)
                self.oled.fill_rect(87, 7, 12, 2, 1)
                self.oled.fill_rect(93, 8, 4, 4, 1)
                self.oled.fill_rect(97, 9, 1, 1, 1)
                self.oled.fill_rect(94, 12, 1, 3, 1)
                self.oled.fill_rect(89, 4, 3, 3, 1)
    ...
    def FillBatteryQuadrants(self, BatteryMethods):
        ...
        elif (4.0 <= BatteryMethods.SMA_battery_voltage <= 4.03):	## if inbetween 80% and 85%
            self.oled.fill_rect(1, 1, 9, 8, 1)		## Fill the 1st quadrant
            self.oled.fill_rect(12, 1, 8, 8, 1)		## Fill the 2nd quadrant
            self.oled.fill_rect(22, 1, 8, 8, 1)		## Fill the 3rd quadrant
            self.oled.fill_rect(32, 1, 3, 8, 1)		## Fill the last quadrant
        elif (4.03 <= BatteryMethods.SMA_battery_voltage <= 4.05):	## if inbetween 85% and 90%
            self.oled.fill_rect(1, 1, 9, 8, 1)		## Fill the 1st quadrant
            self.oled.fill_rect(12, 1, 8, 8, 1)		## Fill the 2nd quadrant
            self.oled.fill_rect(22, 1, 8, 8, 1)		## Fill the 3rd quadrant
            self.oled.fill_rect(32, 1, 5, 8, 1)		## Fill the last quadrant
        elif (4.05 <= BatteryMethods.SMA_battery_voltage <= 4.06):	## if inbetween 90% and 95%
            self.oled.fill_rect(1, 1, 9, 8, 1)		## Fill the 1st quadrant
            self.oled.fill_rect(12, 1, 8, 8, 1)		## Fill the 2nd quadrant
            self.oled.fill_rect(22, 1, 8, 8, 1)		## Fill the 3rd quadrant
            self.oled.fill_rect(32, 1, 6, 8, 1)		## Fill the last quadrant
        elif (4.06 <= BatteryMethods.SMA_battery_voltage <= 4.09 or BatteryMethods.SMA_battery_voltage > 4.09):	## if inbetween 95% and 100%
            self.oled.fill_rect(1, 1, 9, 8, 1)		## Fill the 1st quadrant
            self.oled.fill_rect(12, 1, 8, 8, 1)		## Fill the 2nd quadrant
            self.oled.fill_rect(22, 1, 8, 8, 1)		## Fill the 3rd quadrant
    ...
    def OledSignal(self, BatteryMethods, previous_battery_percent_str, battery_percent_str):
        ...
        if (not BatteryMethods.Check_movingAvgArr()):	## If SMA never occured
            ...
            if (VoltageChangeRate > 0.005 and previous_battery_percent_str != battery_percent_str):
